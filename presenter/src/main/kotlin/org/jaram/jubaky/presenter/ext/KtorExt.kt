...
suspend fun PipelineContext<*, ApplicationCall>.bodyParamListSafe(key: String, default: List<String>? = null): List<String>? {
    return bodyParam()?.getAll(key) ?: default
}

suspend fun PipelineContext<*, ApplicationCall>.bodyParamList(key: String, default: List<String>? = null): List<String> {
    return bodyParamListSafe(key, default) ?: throw MissingParameterException("required $key")
}
