...
import org.jaram.jubaky.JenkinsServerException
...
class JenkinsRepositoryImpl(
    ...
    override suspend fun redirectGithubWebhook(headers: Map<String, String>, payload: String) {
        ...
        if (response.isSuccessful) {
            resultMessage = "Succeed to connect with Github-Webhook"
            logger.info("Github-Webhook : $resultMessage")
        } else {
            resultMessage = "Fail to connect with Github-Webhook"
            logger.error("Github-Webhook : $resultMessage - ${response.errorBody()}")

            throw JenkinsServerException(resultMessage, response.code())
        }
    }
    ...
    override suspend fun getJob(jobName: String): Job? {
        ...
        if (response.isSuccessful) {
            ...
            val healthReportFirstMap = healthReportList?.get(0)
            ...
            val healthScore = healthReportFirstMap?.get("score") as Int?
            val healthDescription = healthReportFirstMap?.get("description") as String?
            ...
            resultMessage = "Succeed to get job info"
            logger.info("$jobName : $resultMessage")

            return jobProtocol.toDomainModel()
        } else {
            resultMessage = "Fail to get job info"
            logger.error("$jobName : $resultMessage - ${response.errorBody()}")
    ...
    override suspend fun getJobSpec(jobName: String, buildNumber: String): JobSpec? {
        ...
        if (response.isSuccessful) {
            ...
            resultMessage = "Succeed to get job spec info"
            logger.info("$jobName/$buildNumber : $resultMessage")

            return jobSpecInfo.toDomainModel()
        } else {
            resultMessage = "Fail to get job spec info"
            logger.error("$jobName/$buildNumber : $resultMessage - ${response.errorBody()}")

            throw JenkinsServerException(resultMessage, response.code())
        }
    }

    override suspend fun getJobLog(jobName: String, buildNumber: String): String? {
        val resultMessage: String
        ...
        if (response.isSuccessful) {
            resultLog = response.body()?: ""

            resultMessage = "Succeed to get job log"
            logger.info("$jobName/$buildNumber : $resultMessage")

            return resultLog
        } else {
            resultMessage = "Fail to get job log"
            logger.error("$jobName/$buildNumber : $resultMessage - ${response.errorBody()}")
    ...
    override suspend fun createJob(jobName: String, configData: JobConfig) {
        ...
        if (response.isSuccessful) {
            resultMessage = "Succeed to create a job"
            logger.info("$jobName : $resultMessage")
        } else {
            resultMessage = "Fail to create a job"
            logger.error("$jobName : $resultMessage - ${response.errorBody()}")
    ...
    override suspend fun deleteJob(jobName: String) {
        ...
        if (response.isSuccessful) {
            resultMessage = "Succeed to delete the job"
            logger.info("$jobName : $resultMessage")
        } else {
            resultMessage = "Fail to delete the job"
            logger.error("$jobName : $resultMessage - ${response.errorBody()}")
    ...
    override suspend fun updateJob(jobName: String, configData: JobConfig) {
        ...
        if (response.isSuccessful) {
            resultMessage = "Succeed to update the job"
            logger.info("$jobName : $resultMessage")
        } else {
            resultMessage = "Fail to update the job"
            response.code()
            logger.error("$jobName : $resultMessage - ${response.errorBody()}")
    ...
    override suspend fun buildWithParameters(jobName: String, buildArgumentList: List<BuildArgument>) {
        ...
        if (response.isSuccessful) {
            resultMessage = "Succeed to build with parameters"
            logger.info("$jobName : $resultMessage")
        } else {
            resultMessage = "Fail to build with parameters"
            logger.error("$jobName : $resultMessage - ${response.errorBody()}")
    ...
    override suspend fun createCredentials(username: String, password: String, key: String, description: String, scope: String) {
        val resultMessage: String
        ...
        credentialsNode.put("scope", scope)
        credentialsNode.put("id", key)
        credentialsNode.put("username", username)
        credentialsNode.put("password", password)
        credentialsNode.put("description", description)
        ...
        if (response.isSuccessful) {
            resultMessage = "Succeed to create a credential - username : $username"
            logger.info("Credentials : $resultMessage")
        } else {
            resultMessage = "Fail to create a credential"
            logger.error("Credentials : $resultMessage - ${response.errorBody()}")
    ...
    override suspend fun deleteCredentials(key: String) {
        ...
        if (response.isSuccessful) {
            resultMessage = "Succeed to delete the credential"
            logger.info("Credentials : $resultMessage")
        } else {
            resultMessage = "Fail to delete the credential"
            logger.error("Credentials : $resultMessage - ${response.errorBody()}")
